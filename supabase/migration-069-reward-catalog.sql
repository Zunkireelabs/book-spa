-- Migration 069: gift card / voucher reward catalog (additive, REVERSIBLE)
--
-- Extends migration-068's referral reward_type (wallet / gift_card / voucher). Wallet rewards
...
--   -- then restore record_customer_referral from migration-068-referral-reward-type.sql
...
-- Drop the migration-068 six-arg overload so callers can't accidentally hit the old
...
VALUES ('069', 'reward-catalog') ON CONFLICT (version) DO NOTHING;
