-- Migration 064: simplify referral reward types to Wallet + Gift Voucher only (additive, REVERSIBLE)
--
-- migration-062/063 introduced three reward types (wallet / gift_card / voucher). Product
...
-- Safe to re-run.
--
-- ROLLBACK: re-run migration-063-reward-catalog.sql's constraint/function definitions to
...
VALUES ('064', 'simplify-reward-types') ON CONFLICT (version) DO NOTHING;
