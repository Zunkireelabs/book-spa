-- Migration 161: allow a custom return time on "Mark Returned Early" (additive, REVERSIBLE)
--
-- revert_staff_transfer_now() (migration-160) always stamped reverted_at (and the synthesized
...
-- three places migration-160 hardcoded v_now (the synthesized return-row's effective_date/
...
--             then re-run migration-160 to restore the 1-arg-only version.
...
VALUES ('161', 'revert-staff-transfer-custom-time')
