-- Migration 063: multi-branch manager access (additive, REVERSIBLE)
--
-- Problem: users.branch_id is a single FK -- a manager can only ever be scoped
-- to one branch. Real need: a manager (e.g. covering two nearby outlets) needs
-- read/write access to more than one branch without juggling two logins.
--
-- Design: users.branch_id stays as the user's "primary" branch (home branch,
-- used for login default / anywhere that expects a single value). A new
-- junction table `user_branches` grants ADDITIONAL branches on top of that.
-- A new array-returning helper `get_user_branch_ids()` unions the primary
-- branch_id with any user_branches rows, and RLS policies that used to check
-- `branch_id = get_user_branch_id()` are swapped to
-- `branch_id = ANY(get_user_branch_ids())`. The old singular
-- `get_user_branch_id()` is untouched -- other call sites still rely on its
-- single-value semantics.
--
-- Granting access is admin-only (this is a permission grant, not something a
-- manager can self-serve). Users can read their own grants.
--
-- Idempotent: CREATE TABLE IF NOT EXISTS, CREATE OR REPLACE FUNCTION,
-- DROP POLICY IF EXISTS + CREATE POLICY, ON CONFLICT DO NOTHING seed insert.
-- Portable: no hardcoded UUIDs (branch/user grants resolved by name/email in
-- the accompanying grant script, not in this migration).
--
-- Reversible:
--   DROP POLICY IF EXISTS "Staff can read branch bookings" ON public.bookings; -- + recreate with get_user_branch_id()
--   ... (revert each swapped policy the same way) ...
--   DROP FUNCTION IF EXISTS public.get_user_branch_ids();
