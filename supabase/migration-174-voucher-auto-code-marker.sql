-- ============================================================
-- Migration 174: distinct marker for auto-generated voucher codes
-- ============================================================
--
-- issue_voucher() supports two code paths: staff type a code from a
-- physical pre-printed booklet (migration-139), or leave it blank and the
-- system auto-generates one from voucher_code_counters as
-- "{code_prefix}-{NNNN}". A real UNIQUE(org_id, voucher_code) constraint
-- (plus migration-140's atomic race-fix) already prevents an actual
-- collision between the two — it just surfaces as a hard "already in use"
-- error at issue time.
--
-- The client wants auto-generated codes to be structurally distinct from a
-- manual/booklet code on sight, so a future booklet print run can never
-- coincide with a system-generated code even by chance. Fix: insert a fixed
-- 'A' marker into the auto-gen branch only — "{code_prefix}-A{NNNN}" instead
-- of "{code_prefix}-{NNNN}". The manual-code branch (p_voucher_code
-- supplied) is completely untouched; staff still type booklet codes exactly
-- as printed. Already-issued vouchers keep their old-format codes — this
-- only changes codes generated going forward, no backfill.
--
-- Base is migration-170 (the current issue_voucher body — migration-140 is
-- NOT the latest revision, migration-169/170 added membership-tender
-- support since then). Same 12-arg signature — CREATE OR REPLACE in place,
...
BEGIN
  ...
  ELSE
    ...
    -- 'A' marker distinguishes an auto-generated code from a manual/booklet
    -- one on sight, so the two can never coincide even by chance.
    v_code := v_type.code_prefix || '-A' || lpad(v_seq::text, 4, '0');
  END IF;
...
-- ============================================================
-- MIGRATION 174 COMPLETE
-- ============================================================

INSERT INTO public.schema_migrations (version, name)
VALUES ('174', 'voucher-auto-code-marker')
