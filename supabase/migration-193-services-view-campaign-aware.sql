-- Migration 193: services_with_offer_pricing campaign-awareness (additive, REVERSIBLE)
--
-- Layers an active-campaign check (migration-192) on top of the existing
-- manual-offer pricing (migration-187/188): if
...
-- migration-189) as long as the existing columns keep their name/type/
...
--
-- RLS: unchanged from migration-188's note — inherited from the underlying
...
--
-- Reversible (manual): re-apply migration-188's CREATE OR REPLACE VIEW
...
VALUES ('193', 'services-view-campaign-aware')
